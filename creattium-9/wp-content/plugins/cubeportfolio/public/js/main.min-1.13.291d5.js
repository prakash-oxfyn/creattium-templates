...
(function($, window, document, undefined) {
    ...
    var processResult = function(result, element) {
            ...
            if (element.data('cbp-selector')) {
                ...
                var skipScriptWithText_three = /window\._tpt|SR7\.|wp-emoji-settings|"prefetch"/;
                ...
                for (i = 0; i < scripts.length; i++) {
					...
					if (!src) {
			            if (!skipScriptWithText_one.test(scripts[i].text) && !skipScriptWithText_two.test(scripts[i].text) && !skipScriptWithText_three.test(scripts[i].text)) {
                ...
                inlineJS += ';\n jQuery(document).ready(function(){ treethemes_special_tabs(true); if (typeof treethemes_init_cube_flexsliders === "function") treethemes_init_cube_flexsliders(".cbp-popup-singlePageInline"); });';
